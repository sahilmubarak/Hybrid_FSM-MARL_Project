...
public class ImpulseShopperBrain : Agent, IShopperBrain
{
    // Tracking variables
    ...
    private void ComputePurchaseRewardAndTrack(int decision)
    {
        ...
        switch (decision)
        {
            case 0: // Buy
                reward = 2;
                if (currentObservations.itemInTheList == 0) reward -= 1f; // Penalty for buying item not from list
                if (currentObservations.TotalItemsBought > currentObservations.ShoppingListCount) reward -= 0.5f; // For over-purchasing
            ...
            case 1: // Browse
                ...
                if (currentObservations.TimeSpent < 150) reward += 1;// Penalty for not spending time
                else if (currentObservations.TimeSpent > 350) reward -= 2f; break;
            case 2: // Ignore
                reward = -0.25f;
                if (currentObservations.itemInTheList == 1) reward = -1; // Penalty for ignoring Shopping list

                if (currentObservations.TimeSpent > 350) reward -= 2f; break;
            default:
                reward = 0f;
                break;
        }
        GiveReward(reward);
        DicisionTracker.Instance.TrackImpulseDecision("Purchase", decision);
    }
    ...
    private void ComputeNavigationRewardAndTrack(int decision)
    {
        ...
        switch (decision)
        {
            ...
            case 3: // Checkout
                reward = 5.0f;
                if (currentObservations.ShoppingProgress < 0.5f) reward = -2.50f; // Penalty for early checkout

                if (currentObservations.TimeSpent < 60) reward -= 2.5f;// Penalty for not spending time
        ...
        GiveReward(reward);
        DicisionTracker.Instance.TrackImpulseDecision("Navigation", decision);
    }
    ...
    private void ComputeDistractionRewardAndTrack(int decision)
    {
        ...
        switch (decision)
        {
            ...
            case 1: // Getting distracted
                ...
                if (currentObservations.DistractingAsileTag == 4) reward += 1.0f;
                else reward += 0.5f;
                break;
            default:
                reward = 0f;
        ...
        GiveReward(reward);
        DicisionTracker.Instance.TrackImpulseDecision("Distraction", decision);
    }
