...
public class WandererShopperBrain : Agent, IShopperBrain
{
    // Tracking variables
    ...
    private void ComputePurchaseRewardAndTrack(int decision)
    {
        ...
        switch (decision)
        {
            case 0: // Buy
                ...
                if (currentObservations.TotalItemsBought > currentObservations.ShoppingListCount / 2) reward -= 2f; // Penalty for over-purchasing(Buying more than half of list)

                if (currentObservations.TimeSpent < 150) reward += 1;// Penalty for not spending time
                else if (currentObservations.TimeSpent > 350) reward -= 1f; 
                break;
            case 1: // Browse
                reward = 1.5f;
                if (currentObservations.TotalItemsBrowsed > 2f * currentObservations.ShoppingListCount) reward = -0.5f; // Penalty for excessive Browse

                if (currentObservations.TimeSpent < 150) reward += 0;// Penalty for not spending time
                else if (currentObservations.TimeSpent > 350) reward -= 1f;
                break;
            case 2: // Ignore
                reward = 0.25f;
        ...
        GiveReward(reward);
        DicisionTracker.Instance.TrackWandererDecision("Purchase", decision);
    }
    ...
    private void ComputeNavigationRewardAndTrack(int decision)
    {
        ...
        switch (decision)
        {
            case 0: // Nearest aisle overall
                reward = 1;
                if (currentObservations.ShoppingProgress < 0.5f) reward = -0.5f;    // Penalty for early checkout
                
                if (currentObservations.TimeSpent < 150) reward = 2f;// spending time
                else if (currentObservations.TimeSpent > 350) reward -= 2f;
                else reward += 1;
               
                break;
            case 1: // Nearest aisle from list
                reward = 1;   // Penalty for early checkout
                
                if (currentObservations.TimeSpent < 150) reward += 2;// Penalty for not spending time
                else if (currentObservations.TimeSpent > 350) reward -= 2;
                else reward += 1;
                
                break;
            case 2: // First aisle in list
                reward = 0.5f;
                if (currentObservations.DistanceToNearestInList < currentObservations.DistanceToNextAisle) reward = -0.5f;  // Penalty for early checkout
            ...
            case 3: // Checkout
                reward = 5.0f;
                if (currentObservations.ShoppingProgress < 0.40f) reward = -2.50f; // Penalty for early checkout

                if (currentObservations.TimeSpent < 100) reward -= 2.5f;// Penalty for not spending time
                else if (currentObservations.TimeSpent > 300) reward += 2.5f;
                else reward += 2;
                break;
            default:
                reward = 0f;
        ...
        GiveReward(reward);
        DicisionTracker.Instance.TrackWandererDecision("Navigation", decision);
    }
    ...
    private void ComputeDistractionRewardAndTrack(int decision)
    {
        ...
        switch (decision)
        {
            case 0: // Not getting distracted
                reward = 1f;
                    if (currentObservations.ShoppingProgress < 0.4f) reward += 1;// Dont get distracted when shopping list not completed
                break;
            case 1: // Getting distracted
                reward = 1.0f; // Impulsive shopper gets rewarded for distraction
                if (currentObservations.DistractingAsileTag != 4) reward -= 0.5f;
                if (currentObservations.TimeSpent < 90) reward += 1f;// Penalty for not spending time
                else if (currentObservations.TimeSpent > 150) reward -= 1f;
                else reward += 0.5f;
                break;
            default:
                reward = 0f;
        ...
        GiveReward(reward);
        DicisionTracker.Instance.TrackWandererDecision("Distraction", decision);
    }
